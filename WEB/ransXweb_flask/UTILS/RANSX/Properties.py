...
class Properties(Calculus, SetAxisLimit, Tools, Errors, object):
    ...
    def __init__(self, filename, plabel, ig, nsdim, ieos, intc, laxis, xbl, xbr):
        ...
        elif plabel == 'oburn' or plabel == '3d-oburn-14ele' or plabel == '3d-oburn-25ele':
            x0002 = self.getRAdata(eht, 'x0002')[intc]  # track prot
        elif plabel == 'neshell' or plabel == '3d-neshellBoost10x-25ele':
            x0002 = self.getRAdata(eht, 'x0002')[intc]  # track
        elif plabel == 'heflash' or plabel == '3d-heflashBoost100x-6ele':
            x0002 = self.getRAdata(eht, 'x0002')[intc]  # track
        elif plabel == 'thpulse' or plabel == '3d-thpulse-15ele':
            x0002 = self.getRAdata(eht, 'x0002')[intc]  # track
        elif plabel == 'cflash':
            x0002 = self.getRAdata(eht, 'x0002')[intc]  # track
        elif plabel == 'heflash' or plabel == '3d-heflashBoost100x-6ele':
    ...
    def properties(self):
        ...
        elif self.plabel == "oburn" or self.plabel == '3d-oburn-14ele' or self.plabel == '3d-oburn-25ele':
        ...
        elif self.plabel == "neshell" or self.plabel == "3d-neshellBoost10x-25ele":
            indCCP = np.where((xzn0 < 3.85e8) & (xzn0 > 3.6e8))[0]
            x0002mean_cnvz = np.mean(self.x0002[indCCP])

        elif self.plabel == "heflash" or self.plabel == '3d-heflashBoost100x-6ele':
            indCCP = np.where((xzn0 < 8.e8) & (xzn0 > 5.e8))[0]
            x0002mean_cnvz = np.mean(self.x0002[indCCP])

        elif self.plabel == "thpulse" or self.plabel == "3d-thpulse-15ele":
        ...
        return {'tauL': tauL, 'kolm_tke_diss_rate': kolm_tke_diss_rate, 'tke_diss': diss, 'tavg': tavg.item(0),
                ...
                'tc': round(tc,1), 'nx': nx.item(0), 'ny': ny.item(0), 'nz': nz.item(0), 'machMax_1': machMax_1,
                'machMean_2': round(machMean_2,3), 'xzn0': xzn0,
                'ig': ig, 'dd': dd, 'x0002mean_cnvz': x0002mean_cnvz, 'pturb_o_pgas': pturb_o_pgas, 'TKEsum': TKEsum,
                'epsD': epsD, 'tD': round(tD,1), 'tenuc': tenuc, 'urms': round(urms,1), 'xznl': xznl, 'xznr': xznr,
                'super_ad_i': super_ad_i, 'super_ad_o': super_ad_o,
                'filename': self.filename, 'timec': round(self.timec, 1),
                'timerange_beg': round(self.trange[0], 1),'timerange_end': round(self.trange[1], 1),'Re': Re,
                'tavg_to': round(tavg.item(0)/tc), 'trange': trange}
