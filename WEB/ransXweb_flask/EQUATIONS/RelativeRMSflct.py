...
class RelativeRMSflct(Calculus, SetAxisLimit, Tools, Errors, object):
    ...
    def plot_relative_rms_flct(self, laxis, bconv, tconv, xbl, xbr, ybu, ybd, ilg, xsc):
        ...
        self.set_plt_axis(laxis, xbl/xsc, xbr/xsc, ybu, ybd, [yval for yval in to_plot])

        # plot DATA 
        plt.title('relative rms fluctuations ' + str(self.nsdim) + "D")
        plt.semilogy(grd1/xsc, plt1, color='brown', label=r"rho")
        plt.semilogy(grd1/xsc, plt2, color='r', label=r"T")
        plt.semilogy(grd1/xsc, plt3, color='g', label=r"P")
        plt.semilogy(grd1/xsc, plt8, color='purple', label=r"Mach")
        ...
        ycol = np.linspace(ybu, ybd, num=100)
        for i in ycol:
