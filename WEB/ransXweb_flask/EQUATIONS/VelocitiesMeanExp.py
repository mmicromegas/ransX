...
class VelocitiesMeanExp(Calculus, SetAxisLimit, Tools, Errors, object):
    ...
    def plot_velocities(self, laxis, bconv, tconv, xbl, xbr, ybu, ybd, ilg, xsc, ysc):
        ...
        if self.ig == 1:
            plt.plot(grd1/xsc, plt1/ysc, color='brown', label=r'ux (Reynolds)')
            plt.plot(grd1/xsc, plt2/ysc, color='red', label=r'ux (Favrian)')
            #plt.plot(grd1/xsc, plt3, color='green', linestyle='--',label=r'$v_{exp}$')
            plt.plot(grd1/xsc, (plt2 - plt1)/ysc, color='m', label=r"-frho/rho")
            # plt.plot(grd1/xsc,plt4,color='blue',label = r'$u_{turb}$')
            plt.plot(grd1/xsc, np.zeros(self.nx), color='k', linestyle='dotted')
        elif self.ig == 2:
            plt.plot(grd1/xsc, plt1/ysc, color='brown', label=r'ur (Reynolds)')
            plt.plot(grd1/xsc, plt2/ysc, color='red', label=r'ur (Favrian)')
            #plt.plot(grd1/xsc, plt3, color='green', linestyle='--',label=r'$v_{exp}$')
            plt.plot(grd1/xsc, (plt2 - plt1)/ysc, color='m', label=r"-frho/rho")
            # plt.plot(grd1/xsc,plt4,color='blue',label = r'$u_{turb}$')
            plt.plot(grd1/xsc, np.zeros(self.nx), color='k', linestyle='dotted')
        ...
        setylabel = r'cm/s (' + "{:.0e}".format(ysc) + ')'
