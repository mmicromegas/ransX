...
class CompareReadParamsRansX:
    ...
    def getForProp(self, param):
        ...
        codes = match[0].split(",")[2:]
        eht_dir = match[1].split(",")[2]
        eht_data_all = match[2].split(",")[2:]
        plabel = match[3].split(",")[2]
        prefix = match[4].split(",")[2]
        ...
        return {'eht_data': eht_data, 'plabel': plabel, 'prefix': prefix, 'ig': ig, 'ieos': ieos, 'intc': intc, 'laxis': laxis,
                'xbl': xbl, 'xbr': xbr, 'code_list': code_list}
