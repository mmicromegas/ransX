...
class TDC(Calculus, Tools, Errors, object):
    ...
    def plot_tempdensx(self, laxis, bconv, tconv, xbl, xbr, ybuBgr1, ybdBgr1, ybuBgr2, ybdBgr2, ybuBgr3, ybdBgr3,
                                                     ilg):
        ...
        # 1st subplot
        fig.add_trace(
            go.Scatter(x=xzn0_l, y=np.log10(self.tt), name=eQterms[0],
                       line=dict(color='brown'),hoverinfo='none'),
            row=1, col=1)

        fig.add_trace(
            go.Scatter(x=xzn0_l, y=np.log10(self.dd), name=eQterms[1],
        ...
        # 2nd subplot
        fig.add_trace(
            go.Scatter(x=xzn0_l, y=self.pp, showlegend=False,
                       line=dict(color='red'),hoverinfo='none'),
            row=1, col=2)
        ...
        fig.update_yaxes(title_text=r"$\mbox{erg g}^{-1} \mbox{cm}^{-3}$", range=[ybdBgr2, ybuBgr2], tickangle=-65,
                         tickwidth=2,
        ...
        # 3rd subplot
        fig.add_trace(
            go.Scatter(x=xzn0_l, y=self.ei,showlegend=False,
                       line=dict(color='red'),hoverinfo='none'),
            row=1, col=3)
        ...
        fig.update_yaxes(title_text=r"$\mbox{erg g}^{-1}$", range=[ybdBgr3, ybuBgr3], tickangle=-45,
                         tickwidth=2,
                         ticklen=10, nticks=10, showgrid=True, exponentformat='e',
                         showline=True, linewidth=1, linecolor='black', mirror=True,
                         ticks='outside', row=1, col=3)
        ...
        fig.update_layout(legend=dict(yanchor="top", y=0.98, xanchor="left", x=0.2, bgcolor='rgba(0,0,0,0)',font=dict(size=18)),
                          title=r"")
