...
class SpectrumTemperatureVariance():

    def __init__(self, filename, data_prefix, ig, lhc):

        block = pd.PROMPI_bindata(filename, ['temp'])

        xzn0 = block.datadict['xzn0']
        temperature = block.datadict['temp']
        ...
        if (ig == 1):
            ...
            print("ERROR (SpectrumTemperatureVariance.py): Spherical Geometry not implemented.")
            # sterad =
            # steradtot =
        else:
            print("ERROR (SpectrumTemperatureVariance.py): Geometry not implemented")

        # get horizontal data
        htemperature = temperature[ilhc][:][:]

        # calculate horizontal mean value
        eh_temperature = np.sum(temperature * sterad) / steradtot

        # calculate Reynolds fluctuations
        temperaturef_r = htemperature - eh_temperature

        # calculate Fourier coefficients (a_ and b_)
        temperaturefr_fft = np.fft.fft2(temperaturef_r)

        a_temperaturef = np.real(temperaturefr_fft)
        b_temperaturef = np.imag(temperaturefr_fft)

        # calculate energy contribution to total variance 
        # from real and imaginary parts of Fourier transforms  

        energy_temperaturef = a_temperaturef * a_temperaturef + b_temperaturef * b_temperaturef
        ...
        for ishell in kh:
            ...
            integrand_ttvar = mask * energy_temperaturef
        ...
        for ishell in kh:
            i += 1
            spect_ttvar_mean.append(spect_ttvar[i] / (float(ny) * float(nz)))

        # check Parseval's theorem
        total_ttvar = (temperaturef_r * temperaturef_r)
        print(np.sum(total_ttvar), np.sum(spect_ttvar))

        # share stuff across class
        self.spect_ttvar = spect_ttvar
        self.spect_ttvar_mean = spect_ttvar_mean
        self.kh = kh
        self.data_prefix = data_prefix

    def plot_TTspectrum(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        """Plot temperature spectrum"""

        # load horizontal wave number GRID
        grd1 = self.kh

        # load spectrum to plot
        plt1 = self.spect_ttvar_mean
        ...
        # plot DATA 
        plt.title('temperature variance "energy" spectrum')
        plt.loglog(grd1, plt1, color='brown', label=r"$T'T'$")
        plt.loglog(grd1, plt1[1] * grd1 ** (-5. / 3.), color='r', linestyle='--', label=r"k$^{-5/3}$")

        setxlabel = r'k$_h$'
        setylabel = r"$P$ variance (K$^{2}$)"
        ...
        # save PLOT
        plt.savefig('RESULTS/' + self.data_prefix + 'ttvariancespectrum.png')
