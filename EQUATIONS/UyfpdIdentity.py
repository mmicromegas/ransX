...
class UyfpdIdentity(calc.Calculus, al.SetAxisLimit, uT.Tools, eR.Errors, object):

    def __init__(self, filename, ig, fext, ieos, intc, data_prefix, bconv, tconv):
        super(UyfpdIdentity, self).__init__(ig)
    ...
    def plot_uyfpd_identity(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        ...
        if self.ig != 1 and self.ig != 2:
            print("ERROR(UyfpdIdentity.py):" + self.errorGeometry(self.ig))
            sys.exit()
        ...
        if self.ig == 1:
            ...
            plt.plot(grd1, plt1, color='r', label=r"$+\overline{u'_y P d}$")
            plt.plot(grd1, plt2, color='g', label=r"$-\overline{P'u'_y} \ \widetilde{d}$")
            plt.plot(grd1, plt3, color='b', label=r"$-\overline{P} \ \overline{u'_y d''}$")
            plt.plot(grd1, plt4, color='m', label=r"$-\overline{P' u'_y d''}$")
        ...
        if self.fext != "png" and self.fext != "eps":
            print("ERROR(UyfpdIdentity.py):" + self.errorOutputFileExtension(self.fext))
            sys.exit()
        ...
        if self.fext == "eps":
            plt.savefig('RESULTS/' + self.data_prefix + 'uyfpd_identity.eps')
