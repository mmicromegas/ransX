...
class AbarResolutionStudy(calc.Calculus, al.SetAxisLimit, uT.Tools, eR.Errors, object):

    def __init__(self, filename, ig, intc, data_prefix):
        super(AbarResolutionStudy, self).__init__(ig)
        ...
        for i in range(len(filename)):
            ...
            abar.append(np.asarray(eht[i].item().get('abar')[intc]))
    ...
    def plot_abar(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        """Plot abar in the model"""

        if (LAXIS != 2):
            print("ERROR(AbarResolutionStudy.py): Only LAXIS=2 is supported.")
            sys.exit()
