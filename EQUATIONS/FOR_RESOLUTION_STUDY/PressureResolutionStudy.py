...
class PressureResolutionStudy(calc.Calculus, al.SetAxisLimit, uT.Tools, eR.Errors, object):

    def __init__(self, filename, ig, intc, data_prefix):
        super(PressureResolutionStudy, self).__init__(ig)
    ...
    def plot_pp(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        """Plot pressure in the model"""

        if (LAXIS != 2):
            print("ERROR(PressureResolutionStudy.py): Only LAXIS=2 is supported.")
            sys.exit()
        ...
        # save PLOT
        plt.savefig('RESULTS/' + self.data_prefix + 'mean_pressure.png')
