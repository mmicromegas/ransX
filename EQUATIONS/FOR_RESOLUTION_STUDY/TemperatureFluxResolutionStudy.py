...
class TemperatureFluxResolutionStudy(calc.Calculus, al.SetAxisLimit, object):
    ...
    def __init__(self,filename,ig,intc,data_prefix):
        ...
        for i in range(len(filename)):			
            ...
            ttux.append(np.asarray(eht[i].item().get('ttux')[intc]))			
            fttx.append(ttux[i] - tt[i]*ux[i])
    ...
    def plot_fttx(self,LAXIS,xbl,xbr,ybu,ybd,ilg):
        ...
        for i in range(len(grd)):
            plt.plot(grd[i],fttx[i],label = str(self.nx[i])+' x '+str(self.ny[i])+' x '+str(self.nz[i]))		
        ...
        # save PLOT
        plt.savefig('RESULTS/'+self.data_prefix+'mean_Temperature_flux.png')
