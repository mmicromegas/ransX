...
class EntropyResolutionStudy(calc.Calculus, al.SetAxisLimit, uT.Tools, eR.Errors, object):

    def __init__(self, filename, ig, intc, data_prefix):
        super(EntropyResolutionStudy, self).__init__(ig)
        ...
        for i in range(len(filename)):
            ...
            ss.append(np.asarray(eht[i].item().get('ddss')[intc])/np.asarray(eht[i].item().get('dd')[intc]))
    ...
    def plot_ss(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        """Plot entropy in the model"""

        if (LAXIS != 2):
            print("ERROR(EntropyResolutionStudy.py): Only LAXIS=2 is supported.")
            sys.exit()
