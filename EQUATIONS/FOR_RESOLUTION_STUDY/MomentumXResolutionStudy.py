...
class MomentumXResolutionStudy(calc.Calculus, al.SetAxisLimit, uT.Tools, eR.Errors, object):

    def __init__(self, filename, ig, intc, data_prefix):
        super(MomentumXResolutionStudy, self).__init__(ig)
        ...
        for i in range(len(filename)):
            ...
            momex.append(np.asarray(eht[i].item().get('ddux')[intc]))
    ...
    def plot_momex(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        """Plot momentumX in the model"""

        if (LAXIS != 2):
            print("ERROR(MomentumXResolutionStudy.py): Only LAXIS=2 is supported.")
            sys.exit()
        ...
        if self.ig == 1:
            setxlabel = r"x (cm)"
            setylabel = r"$\overline{\rho u_x}$ (g cm$^{-2}$ s$^{-1}$)"
        ...
        # save PLOT
        plt.savefig('RESULTS/' + self.data_prefix + 'mean_momentum_x.png')
