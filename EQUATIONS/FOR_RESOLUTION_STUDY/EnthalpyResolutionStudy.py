...
class EnthalpyResolutionStudy(calc.Calculus, al.SetAxisLimit, uT.Tools, eR.Errors, object):

    def __init__(self, filename, ig, intc, data_prefix):
        super(EnthalpyResolutionStudy, self).__init__(ig)
        ...
        for i in range(len(filename)):
            ...
            hh.append(np.asarray(eht[i].item().get('ddhh')[intc])/np.asarray(eht[i].item().get('dd')[intc]))
    ...
    def plot_hh(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        """Plot enthalpy in the model"""

        if (LAXIS != 2):
            print("ERROR(EnthalpyResolutionStudy.py): Only LAXIS=2 is supported.")
            sys.exit()
