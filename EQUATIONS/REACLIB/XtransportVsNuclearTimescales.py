...
class XtransportVsNuclearTimescales(calc.CALCULUS,al.ALIMIT,object):
    ...
    def __init__(self,filename_rans,filename_reaclib,ig,inuc,element,bconv,tconv,tc,intc,data_prefix,network):
    ...
    def plot_Xtimescales(self,LAXIS,xbl,xbr,ybu,ybd,ilg):
        ...
        for rl in rlabel:
            if element in rl:
                rc = rcoeffdict[rl]
                if rl[0:2] == '1-':		
                    plt.plot(grd1,self.GET1NUCtimescale(rc[0],rc[1],rc[2],rc[3],rc[4],rc[5],rc[6]),label=rl,linestyle='--')  
                if rl[0:2] == '2-':
                    rlsplit = rl.split('-')		
                    print(rlsplit)					
                    for elem in rlsplit:				
                    ...
                    if rlsplit[3] == element: 							
                        plt.plot(grd1,self.GET2NUCtimescale(rc[0],rc[1],rc[2],rc[3],rc[4],rc[5],rc[6],fht_yi_list[2],fht_yi_list[0],fht_yi_list[1]),label=rl,linestyle=':')
                    if rlsplit[4] == element: 							
                        plt.plot(grd1,self.GET2NUCtimescale(rc[0],rc[1],rc[2],rc[3],rc[4],rc[5],rc[6],fht_yi_list[3],fht_yi_list[0],fht_yi_list[1]),label=rl,linestyle=':')                				
                ...
                if rl[0:2] == '3-':
                    ...
                    for elem in rlsplit:				
                        if elem in network:
                            ...
                            fht_yi_list.append(((np.asarray(eht.item().get('ddx'+inuc)[intc])/dd)/float(inuc)))
                    if rlsplit[4] == element: 							
                        plt.plot(grd1,self.GET3NUCtimescale(rc[0],rc[1],rc[2],rc[3],rc[4],rc[5],rc[6],fht_yi_list[3],fht_yi_list[4]),label=rl,linestyle=':') 
    ...
    def GET1NUCtimescale(self,c1l,c2l,c3l,c4l,c5l,c6l,c7l):
        ...
        rate = np.exp(c1l + c2l*(temp09**(-1.)) + c3l*(temp09**(-1./3.)) + c4l*(temp09**(1./3.)) + c5l*temp09 + c6l*(temp09**(5./3.)) + c7l*np.log(temp09))
        timescale = 1./(rate)
