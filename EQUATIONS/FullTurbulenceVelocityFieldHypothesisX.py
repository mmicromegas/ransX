...
class FullTurbulenceVelocityFieldHypothesisX(Calculus, SetAxisLimit, Tools, Errors, object):
    ...
    def plot_ftvfhX_equation(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        ...
        if self.ig != 1 and self.ig != 2:
            print("ERROR(FullTurbulenceVelocityFieldHypothesisX.py):" + self.errorGeometry(self.ig))
            sys.exit()
        ...
        # plot DATA
        if self.ig == 1:
            plt.title(r"turbulence velocity field hypothesis X")
            plt.plot(grd1, plt1, color='r', label=r"$+\overline{u'_x \nabla_x u'_x}$")
            plt.plot(grd1, plt2, color='g', label=r"$+\overline{u'_x \nabla_y u'_y}$")
            plt.plot(grd1, plt3, color='b', label=r"$+\overline{u'_x \nabla_z u'_z}$")
            plt.plot(grd1, plt4, color='m',
                     label=r"$-\overline{\rho} \ \overline{u'_x u'_x} \ \widetilde{g}_x/\Gamma_1 \ \overline{P}$")
            ...
            plt.plot(grd1, plt4, color='m',
                     label=r"$-\overline{\rho} \ \overline{u'_r u'_r} \ \widetilde{g}_r/\Gamma_1 \ \overline{P}$")
            plt.plot(grd1, res, color='k', linestyle='--', label=r"$res$")
        ...
        if self.fext != "png" and self.fext != "eps":
            print("ERROR(FullTurbulenceVelocityFieldHypothesisX.py):" + self.errorOutputFileExtension(self.fext))
            sys.exit()
