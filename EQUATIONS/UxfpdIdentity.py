...
class UxfpdIdentity(calc.Calculus, al.SetAxisLimit, uT.Tools, eR.Errors, object):

    def __init__(self, filename, ig, fext, ieos, intc, data_prefix, bconv, tconv):
        super(UxfpdIdentity, self).__init__(ig)
    ...
    def plot_uxfpd_identity(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        ...
        if self.ig != 1 and self.ig != 2:
            print("ERROR(UxfpdIdentity.py):" + self.errorGeometry(self.ig))
            sys.exit()
        ...
        if self.ig == 1:
            ...
            plt.plot(grd1, plt1, color='r', label=r"$+\overline{u'_x P d}$")
            plt.plot(grd1, plt2, color='g', label=r"$-\overline{P'u'_x} \ \widetilde{d}$")
            plt.plot(grd1, plt3, color='b', label=r"$-\overline{P} \ \overline{u'_x d''}$")
            plt.plot(grd1, plt4, color='m', label=r"$-\overline{P' u'_x d''}$")
        ...
        if self.fext != "png" and self.fext != "eps":
            print("ERROR(UxfpdIdentity.py):" + self.errorOutputFileExtension(self.fext))
            sys.exit()
        ...
        if self.fext == "eps":
            plt.savefig('RESULTS/' + self.data_prefix + 'uxfpd_identity.eps')
