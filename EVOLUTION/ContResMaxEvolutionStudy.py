...
class ContResMaxEvolutionResolutionStudy(Calculus, ALIMITevol, Tools, object):

    def __init__(self, filename, ig, data_prefix):
        super(ContResMaxEvolutionResolutionStudy, self).__init__(ig)
        ...
        # declare data lists
        t_timec, t_resContMax, t_tc = [], [], []
        nx, ny, nz, tavg = [], [], [], []

        for i in range(len(filename)):
            # load temporal evolution
            t_timec.append(self.getRAdata(eht[i], 't_timec'))
            t_resContMax.append(self.getRAdata(eht[i], 't_resContMax'))
    ...
    def plot_resContMax_evolution(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        ...
        if (LAXIS != 2):
            print("ERROR(ContResMaxEvolutionStudy.py): Only LAXIS=2 is supported.")
            sys.exit()
        ...
        # plot DATA 
        plt.title('cont res Max evolution')
        ...
        # save PLOT
        plt.savefig('RESULTS/' + self.data_prefix + 'tresContMax_evol.png')
