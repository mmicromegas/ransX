...
class ConvectionBoundariesPositionEvolutionResolutionStudy(Calculus, ALIMITevol, Tools, object):
    ...
    def plot_conv_bndry_location(self, LAXIS, xbl, xbr, ybu, ybd, ilg):
        ...
        for i in range(len(grd)):
            ...
            tc = grd[i]
            ue = str(
                np.format_float_scientific((plotdata[ib[i]] - plotdata[il[i]]) / (tc[ib[i]] - tc[il[i]]), unique=False,
                                           precision=1))
