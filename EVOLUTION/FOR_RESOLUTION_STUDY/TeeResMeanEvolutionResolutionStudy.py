...
class TeeResMeanEvolutionResolutionStudy(uCalc.Calculus, uEal.ALIMITevol, uT.Tools, object):

    def __init__(self, filename, ig, data_prefix):
        super(TeeResMeanEvolutionResolutionStudy, self).__init__(ig)
        ...
        for i in range(len(filename)):
            ...
            t_resTeeMean.append(self.getRAdata(eht[i], 't_resTeeMean'))
        ...
        self.t_resTeeMean = t_resTeeMean
        self.t_tc = t_tc
        self.tavg = tavg
    ...
    def plot_resTeeMean_evolution(self, LAXIS, xbl, xbr, ybu, ybd, ilg):

        grd = self.t_timec
        plt1 = self.t_resTeeMean
        t_tc = self.t_tc
        ...
        if (LAXIS != 2):
            print("ERROR(TeeResMeanEvolutionResolutionStudy.py): Only LAXIS=2 is supported.")
            sys.exit()
        ...
        # plot DATA 
        plt.title('total energy eq res Mean evolution')
        ...
        # save PLOT
        plt.savefig('RESULTS/' + self.data_prefix + 'tresTeeMean_evol.png')
