...
class ResFourierReadParams:
    ...
    def getForProp(self, param):
        ...
        datafiles = match[1].split(",")[2:]
        prefix = match[2].split(",")[2]
        endian = match[3].split(",")[2]
        precision = match[4].split(",")[2]
        ig = match[5].split(",")[2]
        laxis = match[6].split(",")[2]
        lhc = match[7].split(",")[2]

        return {'datadir': datadir, 'datafiles': datafiles, 'prefix': prefix, 'endian': endian, 'precision': precision, 'ig': ig,
                'laxis': laxis, 'lhc': lhc}
