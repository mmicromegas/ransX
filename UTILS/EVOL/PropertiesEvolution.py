...
class PropertiesEvolution(calc.Calculus, al.SetAxisLimit, object):
    ...
    def properties(self, laxis, xbl, xbr):
        ...
        if (self.ig == 1):
            ...
            print(
                "ERROR (PropertiesEvolution.py): geometry not defined, use ig = 1 for CARTESIAN, ig = 2 for SPHERICAL, EXITING ...")
            sys.exit()
        ...
        return {'tauL': tauL, 'kolm_tke_diss_rate': kolm_tke_diss_rate, 'tke_diss': diss, 'tke': tke, 'dd': dd,
                'lc': lc, \
                ...
                'nx': nx, 'ny': ny, 'nz': nz,'machMax': machMax,'machMean': machMean,'x0002mean_cnvz': x0002mean_cnvz}
    ...
    def execute(self):
        ...
        return {'tauL': p['tauL'], 'kolm_tke_diss_rate': p['kolm_tke_diss_rate'], \
                ...
                'pturb_o_pgas': p['pturb_o_pgas'], 'nx': p['nx'], 'ny': p['ny'], 'nz': p['nz'], \
                'machMax': p['machMax'],'machMean': p['machMean'], 'x0002mean_cnvz': p['x0002mean_cnvz']}
