...
class PROMPI_bindata:
    ...
    def __init__(self, filename, ldat):
        ...
        #### below runs in problems with spherical geometry when:
        # zzn 0001-0.40000000E+00-0.39375000E+00-0.38750000E+00
        ### adjust output format in PROMPI, add extra blanks !

        #for i in range(self.qqx):
        #    inline = (f.readline()).split()
        #    #print(i,inline)
        #    self.xznl[i], self.xzn0[i], self.xznr[i] = float(inline[2]),float(inline[3]),float(inline[4])

        #for i in range(self.qqy):
        #    inline = (f.readline()).split()
        #    #print(i,inline)
        #    self.yznl[i], self.yzn0[i], self.yznr[i] = float(inline[2]),float(inline[3]),float(inline[4])

        #for i in range(self.qqz):
        #    inline = (f.readline()).split()
        #    #print(i,inline)
        #    self.zznl[i], self.zzn0[i], self.zznr[i] = float(inline[2]),float(inline[3]),float(inline[4])
        ...
        for dat in ldat:
            ...
            ivar = self.varl.index(dat)
            tmpdata = open(filename, 'rb').read()
            nzones = self.qqx * self.qqy * self.qqz
            fmtspec = str(nzones) + 'f'
            tmpdatb = np.zeros(nzones)
            tmpdatb[:] = struct.unpack(fmtspec, tmpdata[ivar * 4 * nzones:nzones * 4 * (ivar + 1)])
            self.data = tmpdatb.reshape(self.qqz, self.qqy, self.qqx)
