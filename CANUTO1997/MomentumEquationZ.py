...
class MomentumEquationZ(Calculus, SetAxisLimit, Tools, Errors, object):
    ...
    def plot_momentum_z(self, LAXIS, bconv, tconv, xbl, xbr, ybu, ybd, ilg):
        ...
        # save PLOT
        if self.fext == "png":
            plt.savefig('RESULTS/' + self.data_prefix + 'mean_dduz_canuto1997.png')
        if self.fext == "eps":
            plt.savefig('RESULTS/' + self.data_prefix + 'mean_dduz_canuto1997.eps')
    ...
    def plot_momentum_equation_z(self, LAXIS, bconv, tconv, xbl, xbr, ybu, ybd, ilg):
        ...
        if self.ig == 1:
            ...
            plt.plot(grd1, lhs0+lhs1, color='c',label=r"$-\overline{\rho} D_t \widetilde{u}_z $")
            plt.plot(grd1, rhs0, color='b', label=r"$-\nabla_x (\overline{\rho} R_{zx})$")
        ...
        # save PLOT
        if self.fext == "png":
            plt.savefig('RESULTS/' + self.data_prefix + 'momentum_z_eq_canuto1997.png')
        if self.fext == "eps":
            plt.savefig('RESULTS/' + self.data_prefix + 'momentum_z_eq_canuto1997.eps')
